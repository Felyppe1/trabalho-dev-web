...
public class WithdrawController extends HttpServlet {
    ...
    private boolean processWithdraw(String accountId, BigDecimal amount, String description) throws SQLException {
        ...
        String insertTransactionSQL = "INSERT INTO transaction (origin_id, target_id, type, amount, description, balance_after, created_at) VALUES (?, ?, ?, ?, ?, (SELECT balance FROM account WHERE id = ?), ?)";
        ...
        try (Connection conn = PostgresConnection.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement stmt = conn.prepareStatement(insertTransactionSQL)) {
                    stmt.setString(1, accountId); // origin_id is the account making the withdrawal
                    stmt.setString(2, accountId); // target_id is the account for internal withdrawal
                    ...
                    stmt.setString(5, transactionDescription);
                    stmt.setString(6, accountId); // for balance calculation
